...
namespace Engine.Rendering.Temp.Text;
...
public struct Font : IDisposable
{
    ...
    private readonly Dictionary<uint, (Texture, Dictionary<char, FontHelper.Character>)> _cachedAtlases;
    ...
    private Font(uint unicodeRangeStart, uint unicodeRangeEnd, int texWidth, int texHeight, bool autoDispose)
    {
        ...
        _cachedAtlases = new Dictionary<uint, (Texture, Dictionary<char, FontHelper.Character>)>();
    ...
    /// <exception cref="Exception">Thrown if an incorrect parameter is given to the text drawer.</exception>
    public void Draw(int renderer, uint size, string text, Vector2 position, Color startColor,
        float rotation, Vector2 origin, Vector2 scale, int depth = 0, int extraLineSpacing = 0, bool ignoreParams = false, bool smartPlacement = true, int wrapWidth = 0)
    {
        ...
        if (size != _storedSize)
        {
            ...
            {
                (Texture, Dictionary<char, FontHelper.Character>) ch = _cachedAtlases[size];
                _currentTexture = ch.Item1;
                _characters = ch.Item2;
            }
            else
            {
                Texture newTex = new Texture((uint)_texWidth, (uint)_texHeight, 1, WindowClass.Renderer.Device);
    ...
    {
        _storedSize = 0;
        foreach (KeyValuePair<uint, (Texture, Dictionary<char, FontHelper.Character>)> atlas in _cachedAtlases)
