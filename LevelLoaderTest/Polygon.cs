...
namespace Sledge.DataStructures.Geometric.Precision
{
    ...
    public class Polygon
    {
        ...
        public Polygon(Plane plane, double radius = 1000000d)
        {
            ...
            var up = Vector3.Normalize(Vector3.Cross(tempV,plane.Normal));
            var right = Vector3.Normalize(Vector3.Cross(plane.Normal,up));
            ...
            Vertices = verts.Select(x => Vector3.Normalize((x - origin)) * (float)radius + origin).ToList();
        }
        ...
        public bool Split(Plane clip, out Polygon back, out Polygon front, out Polygon coplanarBack, out Polygon coplanarFront)
        {
            ...
            if (cb == 0 && cf == 0)
            {
                ...
                if (Vector3.Dot(Plane.Normal ,clip.Normal) > 0) coplanarFront = this;
            ...
            for (var i = 0; i < Vertices.Count; i++)
            {
                ...
                {
                    var t = sd / (sd - ed);
                    var intersect = s * (float)(1 - t) + e * (float)t;
