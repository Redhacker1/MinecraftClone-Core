...
namespace Engine.Debug
{
    public static class ConsoleLibrary
    {
        ...
        static string scrollback_string = "";
        ...
        public static void process_command(string Text)
        {
            ...
            foreach (List<string> Command in TokenizedCommands)
            {
                ...
                else if (GVars.ContainsKey(CmdName))
                {
                    if (Command.Count == 0)
                    {
                        DebugPrint(GVars[CmdName].ToString());
                    }
                    else
                    {
        ...
        static string ClearScrollback(params string[] Args)
        {
            ...
            {
                _scrollback.Clear();
                scrollback_string = string.Empty;
            }
            return string.Empty;   
        }

        static public ref string GetScrollback()
        {
            return ref scrollback_string;
        }
        ...
        static void Add(string Text)
        {
            lock (WriteLock)
            {
                ...
                scrollback_string = _scrollback.ToString();
            }
        }
