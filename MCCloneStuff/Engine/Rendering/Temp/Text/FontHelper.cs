...
public static class FontHelper
{
    ...
    static FontHelper()
    {
        ...
        foreach (PropertyInfo prop in colorProperties)
            // Probably won't throw nullref exception, but best be safe!
            if (prop.GetValue(null, null) is Color color)
                ColorNames.Add(color.Name.ToLower(), color);
            else
                continue;
    }

    internal static unsafe Dictionary<char, Character> CreateFontTexture(Texture texture, FontFace face,
        uint fontSize, uint asciiRangeStart, uint asciiRangeEnd)
    {
        ...
        for (uint c = asciiRangeStart; c < asciiRangeEnd; c++)
        {
            ...
            if (offsetX + glyph.width >= texture._Texture.Width)
            {
                offsetY += fontSize;
                offsetX = 0;
            }
            ...
            //TODO: Use a single channel texture for this glyph face, we can and we should!
            for (int n = 0; n < 4; n++)
            {
            ...
            texture.UpdateTextureBytes(WindowClass.Renderer.Device, data, glyph.width, glyph.rows );
    ...
    internal static Color GetColor(string name)
    {
        ...
        {
            throw new ($"The given colour is not a valid colour (Color name: '{name}')");
        }
    }
